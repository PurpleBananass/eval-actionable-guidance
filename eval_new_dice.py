...
def _count_flips_for_project_model(
    ...
    """
    Returns (flipped_TPs, unique_testidx_in_file, num_TP, invalid_candidate_rows).
    Logic:
      - Build TP set from get_true_positives (y_true=1 & y_pred=1)
      - Load saved candidates for this project/model/method
      - For each test_idx ∈ TP set, re-predict candidates and count as flipped if ANY predicts class 0
      - invalid_candidate_rows counts rows that fail strict verify (i.e., don't flip)
    """
    ds = read_dataset()
    ...
    # TPs (defects predicted as defects)
    ...
    for t, group in flips.groupby("test_idx"):
        # reconstruct full feature matrix (in case only subset was saved)
...
# Always verify flips using LABELS (consistent across models, esp. SVM).
        ...
        # invalid rows (didn't flip) for diagnostics
        invalid_rows += int(np.sum(preds != 0))
...
def rq1_flip_rates(
    ...
    for m in model_types:
        for method in methods:
            for p in project_list:
                flipped, computed, tp_count, invalid = _count_flips_for_project_model(
                    p, m, method, total_cfs, max_features
                )
                rate = flipped / tp_count if tp_count > 0 else 0.0
                rows.append([m, method, p, flipped, computed, tp_count, rate])
                diag_rows.append([m, method, p, invalid])

    df = pd.DataFrame(rows, columns=["Model", "Method", "Project", "Flip", "Computed", "#TP", "Flip%"])
    diag = pd.DataFrame(diag_rows, columns=["Model", "Method", "Project", "InvalidRows"])

    # Save & print
    Path("./evaluations").mkdir(parents=True, exist_ok=True)
    out = f"./evaluations/flip_rates_DiCE_all_methods_cf{total_cfs}_max{max_features}feat.csv"
    out_diag = f"./evaluations/flip_rates_DiCE_all_methods_cf{total_cfs}_max{max_features}_invalid_rows.csv"
    ...
    print(f"\nSaved to {out}")
    print(f"Diagnostics (non-flipping candidate rows) → {out_diag}")
