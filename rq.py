...
def plot_rq1(results, fname):
    ...
    total_others_df = total_others_df.melt(
        var_name="Explainer", value_name="Flip_Rate", ignore_index=False
    )
    ...
    # Plotting the others
    ax = sns.boxplot(
        x="Explainer", y="Flip_Rate", data=total_others_df, width=0.3, palette="rocket"
    )
    ...
    explainer_order = total_others_df["Explainer"].unique().tolist()
    ...
    plt.tight_layout()
    plt.savefig(
        Path(PLOTS, fname), bbox_inches="tight", dpi=300, transparent=True, format="svg"
    )
    plt.show()
    plt.close()


# plot_rq1(ex1, "RQ4_flippability_min.svg")
...
def incorrectness_per_project(project, explainer):
    ...
    for index, flipped in flipped_instances.iterrows():
        ...
        incorrectness_per_plan = incorrectness(
...
def plot_rq2(results_df):
    ...
    # Plotting the others
    ax = sns.boxplot(
        x="Explainer", y="Ratio", data=others_df, width=0.4, palette="rocket"
    )
    ...
    for explainer in sqa_df["Explainer"].unique():
        for strategy in strategy_order:
            filtered_df = sqa_df[
                (sqa_df["Explainer"] == explainer) & (sqa_df["Strategy"] == strategy)
            ]
    ...
    plt.tight_layout()
    plt.savefig(
        Path(PLOTS) / "RQ1_flippability_DFC.svg",
        bbox_inches="tight",
        dpi=300,
        transparent=True,
        format="svg",
...
# %% RQ3
projects = read_dataset()
baselines = ["DeFlip", "SQAPlanner_coverage"]
data = []
for explainer in baselines:
    for project in projects:
        train, test = projects[project]
        ...
        flipped_instances = pd.read_csv(exp_path, index_col=0)
        if explainer == "DeFlip":
            flipped_instances = flipped_instances.drop("effort", axis=1)
        flipped_instances = flipped_instances.dropna()
        if len(flipped_instances) == 0:
            continue
        scores = []
        ...
        if "SQAPlanner" in explainer:
            ...
            data.append({"Explainer": explainer, "Strategy": None, "Ratio": score})
results_df = pd.DataFrame(data)
results_df.to_csv("./results/rq4.csv")
...
def plot_rq3(results_df):
    ...
    # Define the order and separate the data
    explainer_order = ["DeFlip", "SQAPlanner"]
    strategy_order = ["confidence", "coverage", "lift"]
    others = ["DeFlip", "LIMEHPO", "TimeLIME"]
    sqa = ["SQAPlanner_confidence", "SQAPlanner_coverage", "SQAPlanner_lift"]
    ...
    plt.tight_layout()
    plt.savefig(
        Path(PLOTS) / "RQ4_applicability.svg",
        bbox_inches="tight",
        dpi=300,
        transparent=True,
        format="svg",
