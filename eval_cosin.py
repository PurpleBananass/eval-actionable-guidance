...
def _cosine_distance(u, v):
    ...
    if nu == 0 or nv == 0:
        return 2.0  # max distance on 0–2 scale if degenerate
    s = float(np.dot(u, v) / (nu * nv))  # s in [-1, 1]
    return 1.0 - s  # distance in [0, 2]


def cosine_all(df: pd.DataFrame, x: pd.Series) -> list[float]:
    """
    Compute cosine distances between the query change vector x and each row in df,
    using ONLY the shared columns (df already subset to changed features).
    Returns a list of distances in [0,1]; smaller = more similar.
    """
    # ensure consistent column order
    cols = list(df.columns)
    # align x to the same order, fill missing with 0 just in case
    xv = x.reindex(cols).fillna(0.0).to_numpy(dtype=float)
    distances = []
    for _, row in df.iterrows():
        yv = row.to_numpy(dtype=float)
        distances.append(_cosine_distance(xv, yv))
    return distances
    ...
    for project in project_list:
        ...
        for test_idx in flipped.index:
            if str(test_idx) in plans:
                ...
                # --- in flip_feasibility(...) fix the two branches ---
                ...
                if distance == "cosine":
                    ...
                    if len(nonzero_deltas) < 5:   # <-- was < 0
                        cannot += 1
                        continue
                    distances = mahalanobis_all(nonzero_deltas, changed_flipped)

                # record per-instance stats (distance: smaller = better)
                results.append(
                    {
                        "min": float(np.min(distances)),   # shortest distance = most similar
