...
"""
Nearest-Neighbor Subset CFs (batched, expansion; ≤K by default, NO refinement)

What it does
------------
For each true-positive test instance, search candidates built by copying the instance
and replacing up to K features with values taken from TRAIN neighbors.

- Progressive expansion over: neighbor_k, feature pool size, combinations.
- Mixed-donor fallback (optional): each edited feature can come from a different neighbor.
- Ranking is **distance-only** (primary) with **higher p0** (secondary). No extra heuristics.
- Default policy is **≤K** edits (change fewer features if that yields the nearest flip).
  Pass `--exact_k` to enforce **exactly K** edits.

Distance options (for ranking)
------------------------------
--distance:
  - unit_l2   : Euclidean in median/MAD units of TRAIN (robust). [default]
...
class ModelWrapper:
    ...
    def predict_proba(self, X: NDArray[np.float_]) -> NDArray[np.float_]:
        ...
        if hasattr(self.model, "predict_proba"):
            ...
            if proba.ndim == 1:  # some APIs return P(class1)
                p1 = proba
...
class NeighborIndex:
    """KNN over robust unit space (supports dynamic k)."""
...
def find_cfs_for_instance(
    ...
    # Feature weights (LIME → uniform fallback)
    ...
    def _append_kept(C: np.ndarray):
        ...
        valid = (changed_counts == K) if cfg.exact_k else ((changed_counts >= 1) & (changed_counts <= K))
        if not np.any(valid):
            return
        ...
        # distances (chosen metric)
        dist = _distance_any(cfg.distance, ut, z_mean, z_std, x0, C_sel)

        # Rank: primary distance asc, secondary higher p0
        order = np.lexsort((-P_sel[:, 0], dist))
        C_sel = C_sel[order]; P_sel = P_sel[order]; dist = dist[order]; cc_sel = cc_sel[order]

        for r in range(C_sel.shape[0]):
            rec = {
                **{c: float(v) for c, v in zip(feat_cols, C_sel[r].astype(float))},
                "proba0": float(P_sel[r, 0]),
                "proba1": float(P_sel[r, 1]),
                "num_features_changed": int(cc_sel[r]),
                # legacy column name kept for compatibility; contains the selected distance metric value
                "dist_unit_l2": float(dist[r]),
            }
    ...
    for rd in range(rounds):
        ...
        donors = ut.from_units(knn.Xu[n_idx])  # original space

        # feature pool
        pool_idx = topk_features(weights, max(K, pool_k_r)).tolist()

        # subset sizes and per-size budgets
    ...
    # de-dup identical solutions
    df = df.drop_duplicates(subset=feat_cols, keep="first").reset_index(drop=True)

    # final sort: distance → -proba0
    df = df.sort_values(by=["dist_unit_l2", "proba0"], ascending=[True, False])
...
def main():
    ap = ArgumentParser(description="Nearest-Neighbor Subset CFs (expansion, ≤K by default, no refinement)")
    ...
    ap.add_argument("--top_pool", type=int, default=64)
    ap.add_argument("--neighbor_k", type=int, default=1000)
    ap.add_argument("--max_combos_per_k", type=int, default=60000)
    ap.add_argument("--batch_size", type=int, default=16384)

    # expansion
    ap.add_argument("--expand_rounds", type=int, default=5)
    ap.add_argument("--mixed_donor_after_round", type=int, default=1)
    ap.add_argument("--mixed_donor_reps_per_subset", type=int, default=8)
    ap.add_argument("--mixed_donor_max_candidates", type=int, default=150000)

    # budgets
    ap.add_argument("--instance_time_sec", type=float, default=600.0)
    ap.add_argument("--eval_cap", type=int, default=10000000)
