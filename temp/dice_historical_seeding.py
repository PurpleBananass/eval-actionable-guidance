...
"""
Generate DiCE counterfactuals using historical seeding to improve RQ3 feasibility scores.
Uses similar training instances as starting points for counterfactual generation.

Output CSV (per project/model/method):
  experiments/{project}/{model_type}/{method}/DiCE_all_hist_seed.csv
...
# ----------------------------- historical seeding functions -----------------------------

def get_historical_seeds(target_instance, historical_data, feat_cols, n_seeds=5):
    """Find historically similar instances to use as DiCE initialization seeds."""
    n_seeds = min(n_seeds, len(historical_data))
    
    # Compute distances to all historical instances
    target_array = target_instance[feat_cols].values.reshape(1, -1)
    hist_array = historical_data[feat_cols].values
    distances = euclidean_distances(target_array, hist_array)[0]
    
    # Get indices of closest instances
    closest_indices = distances.argsort()[:n_seeds]
    
    return historical_data.iloc[closest_indices]


def generate_dice_with_historical_seeding(explainer, original_instance, train_data, feat_cols, 
                                         total_cfs=10, method="random", n_seeds=5):
    """Generate counterfactuals using historical seeding."""
    
    # Get historical seeds
    seeds = get_historical_seeds(original_instance, train_data, feat_cols, n_seeds)
    
    all_counterfactuals = []
    cfs_per_seed = max(1, total_cfs // n_seeds)
    
    # Generate from each historical seed
    for _, seed in seeds.iterrows():
        seed_df = seed[feat_cols].to_frame().T
            ...
            if method == "random":
                try:
                    cf = explainer.generate_counterfactuals(
                        seed_df,
                        total_CFs=cfs_per_seed,
                        desired_class="opposite",
                        random_seed=None  # Remove seed for variation
                    )
    ...
    if all_counterfactuals:
        combined = pd.concat(all_counterfactuals, ignore_index=True)
        # Take up to total_cfs candidates
        return combined.iloc[:total_cfs] if len(combined) > total_cfs else combined
    else:
        return None
...
def generate_dice_flips_for_project(project: str,
    ...
    For the given project/model/method:
      ...
      - generate DiCE CFs per TP using historical seeding
      - keep only candidates that actually flip to class 0
      - save to experiments/{project}/{model_type}/{method}/DiCE_all_hist_seed.csv (long format)
    """
    # Validate method
    ...
    # output path - organized by method with new filename to avoid conflicts
    out_dir = Path(EXPERIMENTS) / project / model_type / method
    out_dir.mkdir(parents=True, exist_ok=True)
    out_csv = out_dir / "DiCE_all_hist_seed2.csv"
    ...
    for idx in tqdm(tp_df.index.astype(int), 
        ...
        try:
            # Use historical seeding instead of direct generation
            cf_df = generate_dice_with_historical_seeding(
                explainer, x0, train, feat_cols, total_cfs, method, n_seeds
            )
    ...
    if results:
        ...
        tqdm.write(f"[OK] {project}/{model_type}/{method}: wrote {computed} flipped candidates "
                   f"for {flipped} TP(s) with historical seeding -> {out_csv}")
    else:
        tqdm.write(f"[{project}/{model_type}/{method}] no flipped candidates found with historical seeding.")


# ----------------------------- CLI -----------------------------

def main():
    ap = ArgumentParser(description="Generate DiCE counterfactuals with historical seeding")
    ...
    ap.add_argument("--n_seeds", type=int, default=5,
                    help="Number of historical seeds to use per instance")
    ...
    print(f"Running {len(combos)} combinations with historical seeding:")
    print(f"  Projects: {project_list}")
    print(f"  Models: {model_types}")
    print(f"  Methods: {methods}")
    print(f"  Seeds per instance: {args.n_seeds}")
    print(f"  CFs per instance: {args.total_cfs}")
    print()
