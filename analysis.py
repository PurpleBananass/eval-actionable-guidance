...
def visualize_rq1():
    ...
    for i, container in enumerate(ax.containers):
        ...
        for bar in container:
            ...
            left_text = plt.text(-0.02, bar.get_y() + bar.get_height() / 2, f'{explainers[i]}', va='center', ha='right', fontsize=11)

            value_text = plt.text(bar.get_width() - 0.01, bar.get_y() + bar.get_height() / 2 +0.01 , f'{round(bar.get_width(), 2)}', va='center', ha='right', fontsize=9, fontfamily='monospace')
...
def visualize_rq2():
    explainers = {'LIME':'LIME', 'LIME-HPO':'LIME-HPO', 'TimeLIME':'TimeLIME', 'SQAPlanner': 'SQAPlanner_confidence'}
    ...
    for project in projects:
        ...
        for model_type in models:
            true_positives = get_true_positives(get_model(project, models[model_type]), train, test)
            for explainer in explainers:
                flip_path = Path(EXPERIMENTS) / f"{project}/{models[model_type]}/{explainers[explainer]}_all.csv"
                ...
                unflipped.set_index([unflipped.index, unflipped['project']], inplace=True)
                unflipped = unflipped.drop(columns=['project'])
                total_df = pd.concat([total_df, unflipped[['model', 'explainer', 'score']]], ignore_index=False)
    ...
    for explainer in explainers:
        ...
        for model in models:
            df = total_df[(total_df['explainer'] == explainer) & (total_df['model'] == model)]
    ...
    for e, explainer in enumerate(explainers):
        ...
        for j, model in enumerate(models):
            ...
            sns.histplot(data=df, x='score', ax=ax, color=colors[j], stat='count',  common_norm=False, common_bins=True, cumulative=True, bins=10)
            ...
            for i, container in enumerate(ax.containers):
                for j, bar in enumerate(container):
                    if j == len(container) - 1:
                        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 20, f'.{bar.get_height()/len(df)*100:.0f}', ha='center', va='bottom', fontsize=10, fontfamily='monospace')                    
                    if j == 0:
                        ax.text(bar.get_x() + bar.get_width() / 10 * 3.5, bar.get_height() + 20, f'.{bar.get_height()/len(df)*100:.0f}', ha='center', va='bottom', fontsize=10, fontfamily='monospace')
...
def compare_changes(model="XGBoost", ex1="TimeLIME", ex2="LIME-HPO"):
    df1 = pd.read_csv(f"./evaluations/abs_changes/{model}_{ex1}.csv", names=['score'], header=0)
    df2 = pd.read_csv(f"./evaluations/abs_changes/{model}_{ex2}.csv", names=['score'], header=0)
...
def visualize_implications():
    ...
    for model in models:
        for explainer in explainers:
            df = pd.read_csv(f"./evaluations/abs_changes/{model}_{explainer}.csv", names=['score'], header=0)
            df['Model'] = model
            df['Explainer'] = explainer
...
def visualize_rq3():
    ...
    for model in models:
        for explainer in explainers:
            df = pd.read_csv(f"./evaluations/feasibility/mahalanobis/{models[model]}_{explainer}.csv")
            df['Model'] = model
            df['Explainer'] = explainer
            total_df = pd.concat([total_df, df], ignore_index=True)
    ...
    sns.stripplot(data=test_df, x='Explainer', y='min', hue='Model', palette='crest', dodge=True, jitter=0.2, size=4, alpha=0.25, legend=False)
    ax = sns.pointplot(data=test_df, x='Explainer', y='min', hue='Model', palette=['red', 'red', 'red'], dodge=.8-.8/3, errorbar=None, markers='x', markersize=4, linestyles="none", legend=False, zorder=10)
    ...
    for i, row in mean_df.iterrows():
        ...
        val_str = '.'+val_str[3:]
        if model_idx == 0:
            ax.text(explainers.index(i[1]) - 0.3, row['min']+0.01, val_str, va='bottom', ha='center', fontsize=12, fontfamily='monospace', color='black')
        elif model_idx == 1:
            ax.text(explainers.index(i[1])-0.05,row['min']+0.01, val_str, va='bottom', ha='center', fontsize=12, fontfamily='monospace', color='black')
        else:
            ax.text(explainers.index(i[1]) + 0.25,row['min']+0.01, val_str, va='bottom', ha='center', fontsize=12, fontfamily='monospace', color='black')
    ...
    legend_elements = [Patch(facecolor=colors[i], edgecolor='black', label=models[list(models.keys())[i]]) for i in range(3)]

    fig.legend(handles=legend_elements, title='', loc='upper center', fontsize=12, frameon=False, ncols=3, bbox_to_anchor=(0.525, 0.94))
...
if __name__ == "__main__":
    ...
    if args.implications:
        ...
        print(tabulate(table, headers=["Model", "Explainer 1", "Explainer 2", "p-value", "Effect Size"], tablefmt="grid"))