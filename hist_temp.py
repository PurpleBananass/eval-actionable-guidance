...
def _flip_path(project: str, model_type: str, method: str, use_hist_seed: bool = False) -> Path:
    ...
    filename = "DiCE_all_hist_seed.csv" if use_hist_seed else "DiCE_all_hist.csv"
...
def plan_similarity_dice(project: str, model_type: str, method: str, use_hist_seed: bool = False) -> dict[int, dict]:
    """
    ...
    flip_path = _flip_path(project, model_type, method, use_hist_seed)
...
def rq2_similarity(model_types: list[str], projects: list[str], methods: list[str], use_hist_seed: bool = False):
    ds = read_dataset()
    project_list = list(sorted(ds.keys())) if projects is None else projects
    ...
    for m in model_types:
        for method in methods:
            ...
            for p in project_list:
                d = plan_similarity_dice(p, m, method, use_hist_seed)
                if not d:
                    continue
            ...
            if not all_scores.empty:
                suffix = "_hist_seed" if use_hist_seed else ""
                out = f"./evaluations/similarities/{MODEL_ABBR.get(m, m)}_DiCE_{method}{suffix}.csv"
...
def _flip_feasibility_for_group(
    ...
    *,
    ...
    min_rows=5,                 # keep as you had (legacy often used 5 for Mahalanobis)
    require_all_nonzero=False,  # ROLLED BACK: legacy-lenient default
    ...
    for p in project_list:
        ...
        #  - new: test_idx as a column
        #  - legacy: test ids live in the index
        if "test_idx" in flips.columns:
            group_iter = flips.groupby("test_idx")
        else:
            # treat each unique index as one group (legacy)
        ...
        for test_idx, g in group_iter:
            try:
                ...
                # if legacy index isn't int, skip or try best-effort
                continue
            ...
            for _, cand in g.iterrows():
                # ROLLED BACK: use direct inequality (no math.isclose)
                changed = {
                ...
                # ROLLED BACK: lenient nonzero rule (any nonzero across features)
...
def rq3_feasibility(
    ...
    min_rows: int = 5,
    require_all_nonzero: bool = False,   # ROLLED BACK default here too
    ...
    for m in model_types:
        for method in methods:
            ...
            if all_rows:
                ...
                # ROLLED BACK: NO outlier filtering here

                if len(df) > 0:
                    out = f"./evaluations/feasibility/{distance}/{MODEL_ABBR.get(m, m)}_DiCE_{method}_{selection_strategy}{'_hist_seed3' if use_hist_seed else ''}.csv"
                    df.to_csv(out, index=False)
    ...
    if summary:
        ...
        s.to_csv(f"./evaluations/feasibility_{distance}_DiCE_all_methods_{selection_strategy}{'_hist_seed3' if use_hist_seed else ''}.csv", index=False)
        print("\nFeasibility summary:")
...
def main():
    ...
    ap.add_argument("--use_hist_seed", action="store_true",
                    help="Use historical seeding results (DiCE_all_hist_seed.csv instead of DiCE_all.csv)")
    args = ap.parse_args()
    ...
    hist_tag = " (historical seeding)" if args.use_hist_seed else ""
    print(f"Evaluating {len(model_types)} models × {len(methods)} methods × {len(project_list)} projects{hist_tag}")
    ...
    if args.rq2:
        ...
        rq2_similarity(model_types, project_list, methods, args.use_hist_seed)
